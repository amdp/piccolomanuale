<template>
  <b-container class="text-center">
    <br /><br /><br />
    <h1>Conclusione</h1>
    <h1>
      <nuxt-link to="/piccolomanuale">&lt;</nuxt-link>
      <nuxt-link to="/piccolomanuale/1">></nuxt-link>
    </h1>
    <b-container class="t18 text-justify">
      Partiamo subito da una cosa: nella vita umana ci vuole amore. Nessuno è
      nato senza protezione e nutrimento. Anche se foste stati abbandonati in
      una foresta appena nati, sareste stati amati dalla natura. Questo piccolo
      manuale sulla cooperazione è scritto da una persona ignorante, parziale,
      umana, limitata e probabilmente poco umile. Forse, tra i pro, abbiamo
      buoni propositi e grande dedizione, anche se a fronte di scarse capacità.
      Ma sicuramente quello che mi ha guidato da quando sono nato è l'amore che
      ho ricevuto e l'amore potenziale che ho sempre scorto tra una ingiustizia
      e l'altra che ho incontrato.<br />
      In sintesi, sebbene i tentati atti di umiltà siano stati molti, a quanto
      pare non sono serviti a granché. Ogni umano è parziale, è solo una parte
      in un grande insieme chiamato da noi umani Universo (che come dicevo per
      alcuni non è Uni ma è Multi-verso). Leggete queste pagine, ascoltate
      queste parole prendendole con scetticità, dubbio, mettendole continuamente
      in discussione. Aggreditele, datemi pure dell'idiota o del fautore di una
      chissà quale pseudoscienza o del complottista. Testate ogni cosa esposta,
      ogni allusione, ogni invenzione e cambiatela, anche negli anni a venire,
      registrando però tutte le modifiche, di modo che questo libro abbia una
      vita, degli amici, cresca e si trasformi nell'ambiente come avviene per
      noi umani. Forse un giorno lo cancellerete e morirà.<br />
      Strano a dirsi, ma anche di questo vi sarò eternamente grato, perché qui
      la cosa importante è il libro, e di più l'oggetto del libro, e ancor di
      più la sua storia, non chi l'ha scritto né chi lo modificherà o ne
      scriverà altri, come di ogni umano non è tanto importante chi l'ha fatto o
      chi ne ha modificato la vita, quanto la storia che se ne può raccontare.
      <br />
      <p style="text-align: right">Alessandro Merletti De Palo</p>
    </b-container>
    <br /><br /><br />
    <h1>
      <nuxt-link to="/piccolomanuale">&lt;</nuxt-link>
      <nuxt-link to="/piccolomanuale/1">></nuxt-link>
    </h1>
    <h5>
      <a href="https://creativecommons.org/licenses/by-nc-sa/4.0/">
        Creative Commons BY-NC-SA International License
      </a>
    </h5>
  </b-container>
</template>